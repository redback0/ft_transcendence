				...
				<div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto mt-2 sm:mt-0" style="margin-left:auto; padding-right:1rem;">
					<div class="relative inline-block text-left">
						...
						<div class="dropdown-menu hidden absolute right-0 mt-2 w-32 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10" style="border: 2px solid #DED19C;">
							<button class="block w-full text-left px-4 py-2 text-sm text-[#DED19C] bg-[#000000] hover:bg-[#520404]" style="border-radius:0.375rem 0.375rem 0 0;" data-userid="${friend.id}" data-action="${friend.blocked ? 'unblock' : 'block'}">${friend.blocked ? "UNBLOCK" : "BLOCK"}</button>
							<button class="block w-full text-left px-4 py-2 text-sm text-[#DED19C] bg-[#000000] hover:bg-[#520404]" style="border-radius:0 0 0.375rem 0.375rem;" data-userid="${friend.id}" data-action="defriend">DE-FRIEND</button>
						</div>
					</div>
